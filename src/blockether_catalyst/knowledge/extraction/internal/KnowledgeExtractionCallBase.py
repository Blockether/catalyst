...
class BaseTermExtractionCall(BaseConsensusCall[TermMeaningExtractionResponse]):
    ...
    def fill_template(
        ...
        """Fill template for term extraction prompt."""
        pass
...
class BaseChunkContentClassificationCall(BaseConsensusCall[KnowledgeChunkClassificationResponse]):
    """
...
class BaseTableCaptionExtractionCall(BaseConsensusCall[TableCaptionExtractionResponse]):
    """
    Base class for table caption extraction calls.

    Users inherit from this to generate intelligent table captions with their LLM.
    """
    ...
    def fill_template(
        ...
        Args:
            table_content: First 500 characters of table content in markdown format
            document_name: Name of the document containing the table
...
class ExtractionCallsSettings(BaseModel):
    ...
    table_caption_extraction_call: Optional[BaseTableCaptionExtractionCall] = Field(
        default=None,
        description="Optional user-implemented call for table caption generation",
    )
