...
class ImageRecognition:
    ...
    def _generate_response(self, image: Image.Image, prompt: str, system_prompt: Optional[str]) -> str:
        ...
        try:
            ...
            logger.debug(f"Generating image caption with model {self._settings.model_id} at {self._settings.base_url}")

            # Generate response using the model with configurable timeout
            response = self._llm.chat.completions.create(
                model=self._settings.model_id,
                messages=messages,  # type: ignore[arg-type]
                max_tokens=self._settings.max_tokens,
                temperature=self._settings.temperature,
                timeout=self._settings.timeout,  # Use configurable timeout from settings
            )
            ...
            logger.debug(f"Successfully generated caption: {text_response[:100]}...")
