...
from blockether_catalyst.knowledge.extraction.internal.ImageExtractor import (
    ImageRecognition,
)
...
from blockether_catalyst.knowledge.KnowledgeTypes import (
    ...
    KnowledgeChunkWithSemanticTypes,
    ...
    KnowledgeExtractionResultWithChunksWithTerms,
    KnowledgePageData,
...
from blockether_catalyst.utils import ConcurrentProcessor
...
def timed_operation(step_name: str) -> Callable[[Callable[..., T]], Callable[..., T]]:
    ...
    def decorator(operation_fn: Callable[..., T]) -> Callable[..., T]:
        @wraps(operation_fn)
...
def async_timed_operation(
    ...
    def decorator(operation_fn: Callable[..., T]) -> Callable[..., T]:
        ...
        async def wrapper(*args: Any, **kwargs: Any) -> T:
            start_time = time.time()
            logger.info(f"{step_name}: Starting...")
            result = await operation_fn(*args, **kwargs)  # type: ignore
            elapsed = time.time() - start_time
            logger.info(f"{step_name}: Completed in {elapsed:.2f}s")
...
class KnowledgeExtractionCore:
    ...
    def _process_files_by_extension(self, files_by_extension: dict[str, list[Path]]) -> KnowledgeExtractionOutput:
        ...
        NOTE: PDF files are processed using ProcessPoolExecutor (prefork) due to pypdfium2 thread-safety issues.
        See: https://github.com/pypdfium2-team/pypdfium2/issues/309
        ...
        for extension, file_list in files_by_extension.items():
            ...
            # Process PDF files using ProcessPoolExecutor (prefork) to avoid pypdfium2 threading issues
            if extension == ".pdf":
                logger.info("Processing PDF files with ProcessPoolExecutor (pypdfium2 requires process isolation)")

                # Determine optimal number of workers
                max_workers = min(os.cpu_count() or 4, len(file_list)) if len(file_list) > 0 else 1

                # Use ProcessPoolExecutor for process-based parallelism (prefork)
                with ProcessPoolExecutor(max_workers=max_workers) as executor:
                    # Submit all PDF processing tasks
                    futures_to_files = {}
                    for file_path in file_list:
                        # We need to pass the settings to the subprocess since it won't have access to self
                        future = executor.submit(
                            self._process_single_pdf_in_subprocess,
                            file_path,
                    ...
                    for future in as_completed(futures_to_files):
                        ...
                        try:
                            extraction_item = future.result(timeout=1800)  # 30 minute timeout per file
                            if extraction_item:
                                extraction_results.append(extraction_item)
                                logger.info(f"Successfully extracted: {file_path}")
                        except TimeoutError:
                            logger.error(f"File {file_path} processing timed out after 30 minutes")
            ...
            logger.info(f"Completed processing {len(extraction_results)} {extension} files")
        return extraction_output

    @staticmethod
    def _process_single_pdf_in_subprocess(
        file_path: Path,
        image_output_dir: Path,
        knowledge_settings: "KnowledgeProcessorSettings",
    ) -> Optional[KnowledgeExtractionItem]:
        """Process a single PDF file in a subprocess.

        This static method is used by ProcessPoolExecutor to process PDFs in separate processes,
        avoiding pypdfium2's thread-safety issues.
        ...
        """
        try:
            logger.info(f"Processing PDF in subprocess: {file_path}")

            # Create a new extractor instance in this process
            extractor = PDFKnowledgeExtractor(image_output_dir=image_output_dir, knowledge_settings=knowledge_settings)

            # Extract the PDF
            result = extractor.extract(file_path)

            logger.info(f"Completed processing in subprocess: {file_path}")
            return KnowledgeExtractionItem(result=result)

        except Exception as e:
            logger.error(f"Error processing {file_path} in subprocess: {e}")
            return None
    ...
    def _build_document_chunk_index(
    ...
    ) -> Dict[str, list[RawKnowledgeChunk]]:
        ...
        """
        chunks_index: Dict[str, list[RawKnowledgeChunk]] = {}
    ...
    async def extract(self, globs: list[str]) -> None:
        ...
        extraction_timestamp = int(extraction_start_time)
        ...
        document_to_chunks_index: Dict[str, list[RawKnowledgeChunk]] = self._build_document_chunk_index(
            results_with_chunks
        )
        ...
        # Step 8.5: Enrich chunks with terms
        results_with_chunks_and_terms, document_to_chunks_index_with_terms = self._enrich_chunks_with_terms(
            results_with_classified_chunks,
            consolidated_terms_with_links,
        )

        # Step 9: Build LinkedKnowledge
        def build_linked_knowledge() -> LinkedKnowledge:
            # Calculate processing duration up to this point
            current_duration = int(time.time() - extraction_start_time)
            ...
            return LinkedKnowledge.from_extraction_data(
                raw_data,
                extraction_timestamp=extraction_timestamp,
                processing_duration=current_duration,
            )
        ...
        # Step 11: Copy source documents (now handled by PDF optimization stage in Celery pipeline)
        # Note: Document copying moved to optimize_pdfs_task (Stage 11) to avoid duplication
        ...
        if not search_pickle_path.exists():
            ...
            search_core = KnowledgeSearchCore(
                ...
                pickle_path=str(search_pickle_path),
            )
    ...
    async def _extract_terms_candidates_from_documents(
        ...
        terms = await processor.process(
            ...
            processor_fn=extract_terms_from_document,
        )
    ...
    async def _enrich_with_terms_meanings(
        ...
        document_to_chunks_index: Dict[str, list[KnowledgeChunkWithTerms]],
    ) -> Dict[str, Term]:
        ...
        async def _enrich_term(
            ...
            if not unwrapped_result or unwrapped_result.meaning_status in ["unknown", "generic"]:
                logger.warning(f"Term extraction failed, invalid, or generic for term: {item.term} (status: {unwrapped_result.meaning_status if unwrapped_result else 'None'})")
                return None
            ...
            # On purpose we are doing this so the acronym and keywords are lowercased..
            return Term(
    ...
    def _enrich_chunks_with_terms(
        self,
        results_with_classified_chunks: Sequence[KnowledgeExtractionResultWithClassifiedChunks],
        consolidated_terms_with_links: Dict[str, TermWithLinks],
    ) -> Tuple[
        list[KnowledgeExtractionResultWithChunksWithTerms],
        Dict[str, list[KnowledgeChunkWithTerms]],
    ]:
        """
        Enrich chunks with term occurrences and rebuild the index.

        Args:
            results_with_classified_chunks: Results with semantically classified chunks
            consolidated_terms_with_links: Dictionary of terms with their links

        Returns:
            Tuple of (results with chunks containing terms, updated chunks index)
        """
        results_with_chunks_and_terms = []
        document_to_chunks_index_with_terms: Dict[str, list[KnowledgeChunkWithTerms]] = {}

        for result in results_with_classified_chunks:
            chunks_with_terms = []

            for chunk in result.chunks:
                # Chunks are already properly typed as KnowledgeChunkWithSemanticTypes
                classified_chunk = chunk

                # Count term occurrences in this chunk
                chunk_terms: Dict[str, int] = {}
                chunk_text_lower = classified_chunk.text.lower()

                for term_key in consolidated_terms_with_links.keys():
                    term_lower = term_key.lower()
                    term_count = chunk_text_lower.count(term_lower)
                    if term_count > 0:
                        chunk_terms[term_key] = term_count

                # Create enriched chunk with terms
                chunk_with_terms = KnowledgeChunkWithTerms(
                    document_id=classified_chunk.document_id,
                    document_name=classified_chunk.document_name,
                    doc_id=classified_chunk.doc_id,
                    index=classified_chunk.index,
                    text=classified_chunk.text,
                    page=classified_chunk.page,
                    content_types=classified_chunk.content_types,
                    semantic_types=classified_chunk.semantic_types,
                    terms=chunk_terms,
                )
                chunks_with_terms.append(chunk_with_terms)

            # Create result with enriched chunks using proper type
            result_with_terms = KnowledgeExtractionResultWithChunksWithTerms(
                id=result.id,
                document_filename=result.document_filename,
                document_metadata=result.document_metadata,
                pages=result.pages,
                chunks=chunks_with_terms,
                total_chunks=len(chunks_with_terms),
            ...
            results_with_chunks_and_terms.append(result_with_terms)

            # Update the index
            document_to_chunks_index_with_terms[result.id] = chunks_with_terms

        logger.info(
            f"Enriched {sum(len(r.chunks) for r in results_with_chunks_and_terms)} chunks with term occurrences"
        )
        return results_with_chunks_and_terms, document_to_chunks_index_with_terms
    ...
    async def _chunk_extraction_pages(
        ...
        processor = ConcurrentProcessor[KnowledgePageData, RawKnowledgeChunk](
            concurrency=10,
            max_retries=3,
        )

        # Create a wrapper function that captures the context
        async def process_page(page: KnowledgePageData) -> list[RawKnowledgeChunk]:
    ...
    async def _process_page_wrapper(
        ...
        Args:
            ...
            metadata: Document metadata
        """
        # Create an enriched page with image captions included in text
        enriched_page = KnowledgePageData(
            page=page.page,
            text=self._enrich_page_text_with_images(page),
            images=page.images,
            tables=page.tables,
        )

        # If the page contains no extractable text or structures, fail fast.
        if not enriched_page.text.strip() and not enriched_page.tables and not enriched_page.images:
            raise ValueError(f"No extractable content found for document '{document_name}', page {page.page}.")

        result = await self.calls.document_chunking_call.execute(
            page=enriched_page, document_name=document_name, metadata=metadata
        )
        ...
        for chunk_decision in result.final_response.chunks:
            ...
            chunk_text = chunk_decision.text.strip()
            if not chunk_text:
                raise ValueError(f"Chunker returned empty text for document '{document_name}', page {page.page}")
    ...
    def _enrich_page_text_with_images(self, page: KnowledgePageData) -> str:
        """
        Enrich page text with image captions to make them searchable.

        Uses a special marker format that can be easily filtered from display
        while keeping content searchable.

        Args:
            page: Page data containing text and images
        ...
        captions: list[str] = [img.caption for img in page.images] if page.images else []

        # Use the utility function to enrich the text
        return KnowledgeProcessingUtils.enrich_page_text_with_images(page.text, captions)
    ...
    async def _classify_chunk_content(
    ...
    ) -> Sequence[KnowledgeExtractionResultWithClassifiedChunks]:
        """
        ...
        for doc_result in results_with_chunks:
            classified_chunks: list[KnowledgeChunkWithSemanticTypes] = []

            # Process chunks in batches using ConcurrentProcessor
            processor = ConcurrentProcessor[RawKnowledgeChunk, KnowledgeChunkClassificationResponse](
                concurrency=10,
                max_retries=2,
            )
            ...
            async def classify_single_chunk(
                ...
                if not chunk.text or not chunk.text.strip():
                    ...
                    return KnowledgeChunkClassificationResponse(
                        semantic_types=set(["general"]),
                        reasoning="Empty chunk, defaulted to 'general'.. Make it 150 characters",
                    )
                ...
                semantic_types = set(result.final_response.semantic_types)

                logger.debug(f"Classified chunk {chunk.index} with types: {semantic_types}")
            ...
            classified_chunks_result = await processor.process(
                ...
                processor_fn=classify_single_chunk,
            )

            # Merge raw chunks with classification results
            for raw_chunk, classification_result in zip(doc_result.chunks, classified_chunks_result):
                if classification_result is not None:
                    # Add searchable marker for table of contents chunks
                    chunk_text = raw_chunk.text
                    if "table_of_contents" in classification_result.semantic_types:
                        # Add marker if not already present to make TOC easily searchable
                        if "[TABLE OF CONTENTS]" not in chunk_text and "Table of Contents" not in chunk_text:
                            chunk_text = "[TABLE OF CONTENTS]\n\n" + chunk_text

                    # Create a KnowledgeChunkWithSemanticTypes by combining raw chunk with classification
                    classified_chunk = KnowledgeChunkWithSemanticTypes(
                        ...
                        text=chunk_text,
                        page=raw_chunk.page,
                        content_types=raw_chunk.content_types,
                        semantic_types=list(classification_result.semantic_types),
                    )
                    classified_chunks.append(classified_chunk)

            # Create new result with classified chunks
            classified_result = KnowledgeExtractionResultWithClassifiedChunks(
                id=doc_result.id,
                document_filename=doc_result.document_filename,
                document_metadata=doc_result.document_metadata,
                pages=doc_result.pages,
                chunks=classified_chunks,
                total_chunks=len(classified_chunks),
                total_pages=doc_result.total_pages,
                total_tables=doc_result.total_tables,
                total_images=doc_result.total_images,
    ...
    def _enrich_with_cooccurrences(
        ...
        for term_key, term in grouped_terms.items():
            ...
            if term_key in cooccurrence_scores:
                ...
                # Store ALL co-occurrences - no artificial limits during extraction
                # Display limiting should happen at presentation layer, not data layer
    ...
    def _regenerate_source_documents(self, globs: list[str]) -> None:
        ...
        # Re-copy source documents (handled by Celery pipeline in production)
        # Note: In regeneration mode, we need to copy manually since Celery optimization stages aren't run
