...
class TestRecursiveComparisonWithLLMConsensus:
    ...
    def _create_llm_call(
        ...
        model_name: str,
        temperature: float,
        fixed_responses: Optional[List] = None,
    ) -> ArityOneTypedCall:
        """Create either real or mock LLM call based on USE_REAL_LLM environment variable."""
        use_real_llm = os.environ.get("USE_REAL_LLM", "").lower() == "true"

        if use_real_llm:
            # Set up real LLM connection
            os.environ["INSTRUCTOR_API_BASE_URL"] = "http://localhost:3005/v1"
            os.environ["INSTRUCTOR_API_KEY"] = "test-key"
            return InstructorLLMCall(
            ...
            return MockInstructorLLMCall(
                ...
                temperature=temperature,
                fixed_responses=fixed_responses or [],
            )
    ...
    async def test_keyword_extraction_consensus_with_unordered_derived(self, mock_judge_keyword: Any) -> None:
        ...
        model1_config = ConsensusCore.model(
            id="keyword_model1",
        ...
        model2_config = ConsensusCore.model(
            id="keyword_model2",
        ...
        model3_config = ConsensusCore.model(
            id="keyword_model3",
...
class TestEdgeCasesWithLLMConsensus:
    ...
    async def test_consensus_with_threshold_boundary(self, mock_judge_keyword: Any) -> None:
        ...
        consensus = ConsensusCore.consensus(
            models=[
                ConsensusCore.model(id="b1", executor=model1_call, perspective="", weight_multiplier=1.0),
                ConsensusCore.model(id="b2", executor=model2_call, perspective="", weight_multiplier=1.0),
            ],
            judge=mock_judge_keyword,
            settings=settings,
        )
