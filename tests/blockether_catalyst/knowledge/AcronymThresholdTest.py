...
class TestAcronymThresholdBehavior:
    ...
    def search_module(self, tmp_path: Path) -> KnowledgeSearchCore:
        ...
        return KnowledgeSearchCore.from_pickle(test_pkl, resources_base_url=RESOURCES_URL)
        ...
        for threshold in thresholds:
            ...
            if threshold > 0.6:  # We know RBI scores are around 0.535
                assert len(below_threshold) > 0, (
                    f"All results above threshold {threshold}, " "acronym filtering not working"
                )
    ...
    def test_application_function_compatibility(self, search_module: KnowledgeSearchCore):
        ...
        if results:
            ...
            assert (
                "RBI" in first_content or "Raiffeisen" in first_content
        ...
        for result in results:
            if result.images:
                for img in result.images:
                    ...
                    assert img.href.startswith("http://localhost:8002/os/"), "Image URL doesn't start with correct base"
