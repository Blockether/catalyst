        ...
        mock_target_consensus.call.side_effect = [
            ConsensusResult(
                ...
                final_response=EvaluationResult(
                    ...
                    strengths=[SemanticString(root=s) for s in ["Clear question"]],
                    weaknesses=[SemanticString(root=s) for s in ["No context requested", "Too brief"]],
                    suggested_improvements=[
                        SemanticString(root=s)
                        for s in [
            ...
            ConsensusResult(
                ...
                final_response=EvaluationResult(
                    ...
                    strengths=[SemanticString(root=s) for s in ["Clear", "Detailed", "Contextual"]],
                    weaknesses=[SemanticString(root=s) for s in []],
                    suggested_improvements=[SemanticString(root=s) for s in []],
        ...
        mock_alignment_consensus.call.side_effect = [
            ConsensusResult(
                ...
                final_response=AlignmentFeedback(
                    ...
                    specific_issues=[
                        SemanticString(root=s) for s in ["Missing request for historical context", "Too brief"]
                    ],
        ...
        mock_target_consensus.call.side_effect = [
            ConsensusResult(
                ...
                final_response=EvaluationResult(
                    ...
                    weaknesses=[SemanticString(root=s) for s in ["Lacks detail"]],
                    suggested_improvements=[SemanticString(root=s) for s in ["Add detail"]],
            ...
            ConsensusResult(
                ...
                final_response=EvaluationResult(
                    ...
                    strengths=[SemanticString(root=s) for s in ["Clear", "Detailed"]],
                    weaknesses=[SemanticString(root=s) for s in []],
                    suggested_improvements=[SemanticString(root=s) for s in []],
        ...
        mock_alignment_consensus.call.side_effect = [
            ConsensusResult(
                ...
                final_response=AlignmentFeedback(
                    ...
                    specific_issues=[SemanticString(root=s) for s in ["Too brief"]],
                    improvement_suggestions=[SemanticString(root=s) for s in ["Add detail request"]],
                    principles_to_apply=AlignmentPrincipleList(
                        principles=[
                            AlignmentPrinciple(
    ...
    async def test_max_iterations_limit(
        ...
        mock_target_consensus.call.return_value = ConsensusResult(
            ...
            final_response=EvaluationResult(
                ...
                weaknesses=[SemanticString(root=s) for s in ["Many issues"]],
                suggested_improvements=[SemanticString(root=s) for s in ["Keep trying"]],
        ...
        mock_alignment_consensus.call.return_value = ConsensusResult(
            ...
            final_response=AlignmentFeedback(
                ...
                specific_issues=[SemanticString(root=s) for s in ["Still not aligned"]],
                improvement_suggestions=[SemanticString(root=s) for s in ["Try again"]],
    ...
    async def test_metrics_calculation(
        ...
        mock_target_consensus.call.side_effect = [
            ConsensusResult(
                ...
                final_response=EvaluationResult(
                    ...
                    strengths=[SemanticString(root=s) for s in []],
                    weaknesses=[SemanticString(root=s) for s in ["Many"]],
                    suggested_improvements=[SemanticString(root=s) for s in ["Improve"]],
            ...
            ConsensusResult(
                ...
                final_response=EvaluationResult(
                    ...
                    strengths=[SemanticString(root=s) for s in ["Some"]],
                    weaknesses=[SemanticString(root=s) for s in ["Few"]],
                    suggested_improvements=[SemanticString(root=s) for s in ["Continue"]],
            ...
            ConsensusResult(
                ...
                final_response=EvaluationResult(
                    alignment_score=0.85,
                    feedback="Good alignment achieved",
                    strengths=[SemanticString(root=s) for s in ["Many"]],
                    weaknesses=[SemanticString(root=s) for s in []],
                    suggested_improvements=[SemanticString(root=s) for s in []],
        ...
        mock_alignment_consensus.call.return_value = ConsensusResult(
            ...
            final_response=AlignmentFeedback(
                ...
                specific_issues=[SemanticString(root=s) for s in ["Issues"]],
                improvement_suggestions=[SemanticString(root=s) for s in ["Suggestions"]],
                principles_to_apply=AlignmentPrincipleList(principles=[]),
